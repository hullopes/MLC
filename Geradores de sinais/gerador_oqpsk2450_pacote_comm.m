spc = 8;%symbols per chip/samples per symbol
SNR = -0;%dB
channel = comm.AWGNChannel('NoiseMethod','Signal to noise ratio (SNR)','SNR',SNR);
amostras = 1000;

fileID = fopen(['~/Documentos/MLC/Dados/oqpsk/oqpsk2450MHz_SNR_' num2str(SNR) '_dBm_' num2str(64) '_bits_' num2str(amostras) '_amostras_pacote_comm_SPS8'  '.txt'],'w');
...
for count = 1:1:amostras
    ...
    dataIn = randi([0 1],898,1); %generate vector of binary data

    %sinal =lrwpan.PHYGeneratorOQPSK(dataIn,spc,'2450 MHz');
    sinal = oqpskmod(dataIn);
